...
func RelaySwapFace(c *gin.Context, info *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	defer func() {
		if mjResp.StatusCode == 200 && mjResp.Response.Code == 1 {
			...
			logContent := fmt.Sprintf("模型固定价格 %.2f，分组倍率 %.2f，用户倍率 %.2f，操作 %s", priceData.ModelPrice, priceData.GroupRatioInfo.GroupRatio, priceData.UserRatio, constant.MjActionSwapFace)
...
func RelayMidjourneySubmit(c *gin.Context, relayInfo *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	defer func() {
		if consumeQuota && midjResponseWithStatus.StatusCode == 200 {
			...
			tokenName := c.GetString("token_name")
			logContent := fmt.Sprintf("模型固定价格 %.2f，分组倍率 %.2f，用户倍率 %.2f，操作 %s，ID %s", priceData.ModelPrice, priceData.GroupRatioInfo.GroupRatio, priceData.UserRatio, midjRequest.Action, midjResponse.Result)
