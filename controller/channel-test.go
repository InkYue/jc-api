...
func settleTestQuota(info *relaycommon.RelayInfo, priceData types.PriceData, usage *dto.Usage) (int, *billingexpr.TieredResult) {
	...
	quota := 0
	userRatio := common.NormalizeQuotaMultiplier(priceData.UserRatio)
	groupRatio := priceData.GroupRatioInfo.GroupRatio
	if !priceData.UsePrice {
		quota = usage.PromptTokens + int(math.Round(float64(usage.CompletionTokens)*priceData.CompletionRatio))
		quota = int(math.Round(float64(quota) * priceData.ModelRatio * groupRatio * userRatio))
	...
	return int(priceData.ModelPrice * common.QuotaPerUnit * groupRatio * userRatio), nil
}
